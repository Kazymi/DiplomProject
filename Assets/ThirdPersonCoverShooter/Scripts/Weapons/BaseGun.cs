    ...
    public abstract class BaseGun : MonoBehaviour
    {
        ...
        private bool fire(float delay, bool consume)
        {
            ...
            if (!isFriend)
            {
                ...
                {
                    GameObject curentDecal = Instantiate<GameObject>(PlayerLibrary.PlayerGameObject.GetComponent<PlayerElement>().Decal);
                    curentDecal.transform.position = hit.point + hit.normal * .03f;
                    curentDecal.transform.SetParent(hit.transform.parent);
                    curentDecal.transform.rotation = Quaternion.LookRotation(-hit.normal);
                }
                if (hit.collider == null)
